...
func TestLoad_Success(t *testing.T) {
	...
	keysToBackup := []string{
		...
		"AWS_USE_STUB", "AWS_REGION", "SNS_TOPIC_ARN", "SQS_QUEUE_URL",
		"OUTBOX_POLL_INTERVAL", "OUTBOX_BATCH_SIZE", "ORG_MEMBERSHIP_BASE_URL",
		"EXECUTION_SERVICE_ADDR",
	}
	...
	_ = os.Setenv("SQS_QUEUE_URL", "https://sqs.queue")
	...
	if cfg.SQSQueueURL != "https://sqs.queue" {
		t.Errorf("SQSQueueURL = %q, want https://sqs.queue", cfg.SQSQueueURL)
	}
...
func TestLoad_ValidationFailures(t *testing.T) {
	envBackup := make(map[string]string)
	keysToBackup := []string{"DATABASE_URL", "AWS_USE_STUB", "SNS_TOPIC_ARN", "SQS_QUEUE_URL"}
	for _, k := range keysToBackup {
		envBackup[k] = os.Getenv(k)
	}
	defer func() {
	...
	tests := []struct {
		...
		{
			name: "missing sqs queue when aws stub is false",
			env: map[string]string{
				"DATABASE_URL":  "postgres://localhost",
				"AWS_USE_STUB":  "false",
				"SNS_TOPIC_ARN": "topic",
		...
		{
			name: "valid when aws stub is true even without sns/sqs",
			env: map[string]string{
				"DATABASE_URL":  "postgres://localhost",
				"AWS_USE_STUB":  "true",
				"SNS_TOPIC_ARN": "",
...
func TestGetEnvDefaults_InvalidValues(t *testing.T) {
	envBackup := make(map[string]string)
	...
	// Should fallback to default values on invalid input format
