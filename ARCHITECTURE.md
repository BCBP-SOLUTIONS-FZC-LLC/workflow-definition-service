...
flowchart LR
    ...
    SNS --> DOWNSTREAM
    SQS -->|"DepartmentMembershipRevoked"| DEF
...
### 3. SQS DepartmentMembershipRevoked

```mermaid
sequenceDiagram
    participant SQS as AWS SQS
    participant Consumer as platform-events SQSConsumer
    participant Handler as adapter/inbound/sqs
    ...
    Consumer ->>+ Handler: HandleMembershipRevoked(ctx, envelope)
    Handler ->> Handler: RecordIfNew(eventID) — SQS dedup guard
    ...
    Svc -->>- Handler: nil
    Handler -->>- Consumer: nil (message deleted)
```
...
| `SQS_QUEUE_URL` | — | SQS queue for membership revocation events |
| `SQS_CONCURRENCY` | `1` | Concurrent SQS message handlers |
