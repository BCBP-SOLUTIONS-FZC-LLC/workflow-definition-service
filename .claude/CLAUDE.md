...
20. **HTTP idempotency middleware** — `handler.WithIdempotency(cache, log, fn)` in `internal/adapter/inbound/http/handler/idempotency.go`. Applied to all mutation endpoints at the router level via an `idem(fn)` wrapper in `cmd/server/router.go`. Cache key is `"idem:" + tenantID + ":" + routePath + ":" + Idempotency-Key` header value — tenant- and route-scoped to prevent cross-endpoint key collisions. Only 2xx responses are cached (24 h TTL). On cache miss the response AND a SHA-256 hash of the raw request body are stored together. On replay (cache hit): if the stored hash matches the incoming body hash, the cached response is replayed; if hashes differ, the middleware returns `IDEMPOTENCY_KEY_REPLAY` (409) before calling the handler. Passes through transparently if the header is absent or cache is nil. A `port.Logger` is injected for warn-level observability on body-read failures and cache-set failures.
