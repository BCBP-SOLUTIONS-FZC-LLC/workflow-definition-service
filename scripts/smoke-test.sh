...
# SQS DepartmentMembershipRevoked consumer — including error cases and idempotency.
#
...
SQS_QUEUE="${SQS_QUEUE:-http://localhost:4566/000000000000/membership-wf-q}"
...
wait_http "${SQS_ENDPOINT}/_localstack/health"    "LocalStack"
...
STATUS=$(api_status PATCH "${API}/workflows/${WF_ID}" '{"name":"smoke-test-updated"}')
[ "$STATUS" = "200" ] && pass "PATCH /workflows/:id → 200" || fail "PATCH /workflows/:id → $STATUS"
...
if [ "$HAS_AWS" = true ]; then
  # Create a fresh workflow in DRAFT state — alice is a default assignee
  WF2_RESP=$(api_post "${API}/workflows" \
    '{"name":"sqs-draft-test","description":"sqs draft invalidation","business_key":"sqs-draft-wf"}')
  WF2_ID=$(get_id "$WF2_RESP")
  V2_DRAFT_RESP=$(api_post "${API}/workflows/${WF2_ID}/draft" \
    "{\"bpmn_xml\":${BPMN_JSON}}")
  V2_DRAFT_ID=$(get_id "$V2_DRAFT_RESP")
  [ -n "$V2_DRAFT_ID" ] && pass "Setup: created draft workflow for SQS test" || fail "Setup: draft creation failed"
...
JSON
  ...
  aws --endpoint-url "${SQS_ENDPOINT}" --region us-east-1 sqs send-message \
    --queue-url "${SQS_QUEUE}" \
    --message-body "$REVOKE_MSG" --output text > /dev/null
  pass "Sent DepartmentMembershipRevoked (alice/design/preparer, id=${EVENT1_ID})"

  echo "  Waiting 4s for consumer..."
  sleep 4
  ...
  if [ "$HAS_PSQL" = true ]; then
    ...
    OUTBOX_COUNT=$(psql "$DB_URL" -tAc \
      "SELECT COUNT(*) FROM outbox_events WHERE payload::text LIKE '%${V2_DRAFT_ID}%'" 2>/dev/null || echo "?")
    [ "$OUTBOX_COUNT" = "0" ] \
      && pass "No outbox event for DRAFT invalidation (correct — DRAFT doesn't emit)" \
      || fail "outbox_events count=${OUTBOX_COUNT} for DRAFT (want 0)"
  else
  ...
  [ "$STATUS" = "200" ] && pass "Setup: published version for SQS published-invalidation test" \
...
JSON
  ...
  pass "Sent DepartmentMembershipRevoked (bob/design/reviewer, id=${EVENT2_ID})"
  ...
  if [ "$HAS_PSQL" = true ]; then
    ...
    PROC_COUNT=$(psql "$DB_URL" -tAc \
      "SELECT COUNT(*) FROM processed_events WHERE event_id='${EVENT2_ID}'" 2>/dev/null || echo "?")
    [ "$PROC_COUNT" = "1" ] \
      && pass "processed_events contains event_id=${EVENT2_ID}" \
  ...
  aws --endpoint-url "${SQS_ENDPOINT}" --region us-east-1 sqs send-message \
    --queue-url "${SQS_QUEUE}" \
    --message-body "$REVOKE_MSG2" --output text > /dev/null
  pass "Re-sent same event (id=${EVENT2_ID}) for idempotency check"
  echo "  Waiting 4s..."
  sleep 4
  ...
  if [ "$HAS_PSQL" = true ]; then
    PROC_AFTER=$(psql "$DB_URL" -tAc \
      "SELECT COUNT(*) FROM processed_events WHERE event_id='${EVENT2_ID}'" 2>/dev/null || echo "?")
    [ "$PROC_AFTER" = "1" ] \
      && pass "processed_events still exactly 1 row (idempotent)" \
      || fail "processed_events count=${PROC_AFTER} after re-delivery (want 1)"

    PUB_OUTBOX2=$(psql "$DB_URL" -tAc \
      "SELECT COUNT(*) FROM outbox_events WHERE payload::text LIKE '%${V3_DRAFT_ID}%'" 2>/dev/null || echo "?")
    [ "$PUB_OUTBOX2" = "1" ] \
      && pass "No duplicate outbox event (idempotent)" \
      || fail "outbox_events count=${PUB_OUTBOX2} after re-delivery (want 1)"
  fi
else
