...
// txPassthrough returns a MockTransactor whose RunInTx / RunInTxWithRetry
// immediately invoke fn (both are passthroughs for unit tests).
func txPassthrough(ctrl *gomock.Controller) *mocks.MockTransactor {
	...
	tx.EXPECT().RunInTx(gomock.Any(), gomock.Any()).AnyTimes().DoAndReturn(passthrough)
	tx.EXPECT().RunInTxWithRetry(gomock.Any(), gomock.Any()).AnyTimes().DoAndReturn(passthrough)
	return tx
}
...
func TestWorkflowService_Create_QuotaExceeded(t *testing.T) {
	ctrl := gomock.NewController(t)
	wfRepo := mocks.NewMockWorkflowRepository(ctrl)
	// Quota COUNT now runs inside the (serializable) tx.
	svc := service.NewWorkflowService(service.WorkflowDeps{Transactor: txPassthrough(ctrl), Workflows: wfRepo})
...
func TestWorkflowService_Archive_OK(t *testing.T) {
	...
	svc := service.NewWorkflowService(service.WorkflowDeps{
		Transactor: tx, Workflows: wfRepo, Versions: vRepo, Execution: exec,
	})
...
func TestWorkflowService_Archive_OK_WithCache(t *testing.T) {
	ctrl := gomock.NewController(t)
	tx := mocks.NewMockTransactor(ctrl)
	wfRepo := mocks.NewMockWorkflowRepository(ctrl)
	vRepo := mocks.NewMockWorkflowVersionRepository(ctrl)
	...
	tenantID, userID, wfID, activeVID := uuid.New(), uuid.New(), uuid.New(), uuid.New()
	svc := service.NewWorkflowService(service.WorkflowDeps{
		Transactor: tx, Workflows: wfRepo, Versions: vRepo, Execution: exec, Cache: cache,
	})
	...
	vRepo.EXPECT().Archive(gomock.Any(), tenantID, activeVID).Return(nil)
	wfRepo.EXPECT().UpdateActiveVersion(gomock.Any(), tenantID, wfID, (*uuid.UUID)(nil)).Return(nil)
	cache.EXPECT().Del(gomock.Any(), gomock.Any()).Return(nil)
...
func TestWorkflowService_Archive_CacheDelError_Logs(t *testing.T) {
	ctrl := gomock.NewController(t)
	tx := mocks.NewMockTransactor(ctrl)
	wfRepo := mocks.NewMockWorkflowRepository(ctrl)
	vRepo := mocks.NewMockWorkflowVersionRepository(ctrl)
