...
- OR:
- [ ] Migration file added to `db/migrations/` with both `.up.sql` and goose annotations
- [ ] Migration is safe to run on a live database (backward-compatible, no locking full-table rewrites on large tables)
- [ ] `make migrate-up` tested locally against a fresh database
...
- [ ] SQS consumer handler is idempotent (checks `processed_event` table before acting)
