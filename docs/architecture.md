...
```sh
cmd/server/             ← bootstrap + DI wire-up only (main.go, wire.go, app.go, infra.go, migrate.go)
│
├── internal/core/
...
│
├── internal/bpmn_compiler/         ← root: Compiler, parser, security scan, hasher, stage-type registry
│   ├── bpmncore/                   ← shared types, traversal engine, Compile/QualifyPlanDepts
│   ├── validator/                  ← all structural/semantic/topological validation functions
│   └── element/                    ← ElementHandler implementations per BPMN node type
...
sequenceDiagram
    ...
    participant SC as Shared Workflow-Events Consumer
    participant D as http/handler.HandleInternalEvent
    participant S as VersionService
    participant DB as PostgreSQL
    participant SNS as SNS (via outbox.Runner)

    SQS->>SC: DepartmentMembershipRevoked envelope
    SC->>D: POST /internal/events (envelope)
    D->>S: HandleMembershipRevoked(eventID, tenantID, userID, deptID)
    S->>DB: RecordIfNew(eventID, "membership-wf-q") — ON CONFLICT DO NOTHING
    DB-->>S: isNew=true
