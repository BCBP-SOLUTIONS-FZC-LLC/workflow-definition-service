...
Migrations use **Goose** — not golang-migrate. Every migration file requires both `Up` and `Down` steps:
...
- Always test `migrate-down` as well as `migrate-up`.
- Migrations that lock large tables (e.g. `ADD COLUMN NOT NULL DEFAULT`) must be split into multiple safe steps (add nullable → backfill → add constraint).
...
| ----------------- | -------- | --------- |
| `gen/proto/` | `proto/*.proto` | `make generate-proto` |
