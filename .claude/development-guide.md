...
- **New event type**: define the payload in `internal/core/domain/eventpayloads.go`, add the message + `x-lifecycle`/`x-owner` block to `api/asyncapi.yaml`, run `make extract-schemas` to derive `internal/eventschema/<name>.json`, run `make schema-validate` before committing, enqueue via `outbox.Enqueue` inside a `pgcommon.RunInTx` callback — never call `publisher.Publish` directly.
