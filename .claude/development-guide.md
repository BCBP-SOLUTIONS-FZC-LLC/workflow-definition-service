...
- **New event type**: `internal/core/domain/eventpayloads.go` now only holds `EventSource` — a payload shared with the Execution Service is a change in the separate `workflow-models` repo/release (`pkg/events`/`pkg/enums`), not a local file edit; a Definition-only payload's struct still belongs here. Either way: add the message + `x-lifecycle`/`x-owner` block to `api/asyncapi.yaml`, run `make extract-schemas` to derive `internal/eventschema/<name>.json`, run `make schema-validate` before committing, enqueue via `outbox.Enqueue` inside a `pgcommon.RunInTx` callback — never call `publisher.Publish` directly.
