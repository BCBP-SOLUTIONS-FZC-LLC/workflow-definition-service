...
var testGinConfig = gincommon.Config{ServiceName: "definition-service-test"}

type fakePinger struct{ err error }

func (f fakePinger) Ping(context.Context) error { return f.err }

type fakeDBPinger struct{ health DBHealth }

func (f fakeDBPinger) Health(context.Context) DBHealth { return f.health }

func testRouter(db DBPinger, cache Pinger) *gin.Engine {
...
func TestReadyz_AllHealthy_Returns200(t *testing.T) {
	r := testRouter(fakeDBPinger{health: DBHealth{Healthy: true}}, fakePinger{})
...
func TestReadyz_DBUnhealthy_Returns503(t *testing.T) {
	r := testRouter(fakeDBPinger{health: DBHealth{Healthy: false}}, fakePinger{})
...
func TestReadyz_CacheUnreachable_Returns503(t *testing.T) {
	r := testRouter(fakeDBPinger{health: DBHealth{Healthy: true}}, fakePinger{err: errors.New("valkey down")})
...
func TestNewRouter_Healthz(t *testing.T) {
	r := testRouter(fakeDBPinger{}, fakePinger{})
...
func TestNewRouter_InternalRoutesRequireToken(t *testing.T) {
	...
	router := NewRouter(RouterConfig{
		...
		Handler:          handler.New(handler.Services{}),
		DB:               fakeDBPinger{}, Cache: fakePinger{},
	})
